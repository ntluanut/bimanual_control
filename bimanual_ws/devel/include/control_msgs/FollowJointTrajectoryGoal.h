...
struct Printer< ::control_msgs::FollowJointTrajectoryGoal_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::control_msgs::FollowJointTrajectoryGoal_<ContainerAllocator>& v)
  {
    ...
    if (v.path_tolerance.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.path_tolerance.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::control_msgs::JointTolerance_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.path_tolerance[i]);
    }
    if (v.path_tolerance.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    if (v.goal_tolerance.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.goal_tolerance.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::control_msgs::JointTolerance_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.goal_tolerance[i]);
    }
    if (v.goal_tolerance.empty() || false)
      s << "]";
    if (true || !indent.empty())
